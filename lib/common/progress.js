...
module.exports = function createProgress(title, message){
  ...
  function drawBarLine(fill, str){
    ...
    return (
      chalk.blue(repeatStr(BAR_CHAR, fill)) +
      chalk.white(repeatStr(BAR_CHAR, BAR_LENGTH - fill)) +
      ' ' + boxStr(safeStr(str || ''), MAX_LINE_WIDTH - x - BAR_LENGTH - 2)
    );
  }
  ...
  return {
    update: function(percent, message){
      ...
      draw(x, lines - line, boxStr(message, MAX_LINE_WIDTH - x));
    }
  };
};
