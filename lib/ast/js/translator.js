...
var OPERATORS = ['in', 'instanceof', 'typeof', 'new', 'void', 'delete', '++', '--', '+', '-', '!', '~', '&', '|', '^', '*', '/', '%', '>>', '<<', '>>>', '<', '>', '<=', '>=', '==', '===', '!=', '!==', '?', '=', '+=', '-=', '/=', '*=', '%=', '>>=', '<<=', '>>>=', '|=', '^=', '&=', '&&', '||'].reduce(function(res, key){
  res[key] = true;
  return res;
}, {});
      ...
      var bodyCode = n == 0
        ...
        : '{' + newline +
            body.map(function(branch, i){
              var body = branch[1];
              var code = branch[0]
                ? indent_(+1, true) + add_spaces(['case', walk(branch[0]) + ':'])
                : indent_(+1, true) + 'default:';

              if (body.length > 0)
              {
                indent_(+2);
                code += newline + make_block_statements(body).join(newline);
                indent_(-2);
