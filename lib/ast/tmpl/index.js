...
function getContext(module, extContext){
  ...
  {
    var tmpl = module;
    var ATTRIBUTES_AND_CHILDREN = tmpl.ELEMENT_ATTRIBUTES_AND_CHILDREN || tmpl.ELEMENT_ATTRS;
    var typeMap = {};
    ...
    BaseContext.prototype = {
      ...
      getAttrs: function(token){
        if (token[tmpl.TOKEN_TYPE] == tmpl.TYPE_ELEMENT)
          switch (this.version)
          {
            case 1:
              return token[tmpl.ELEMENT_ATTRS];

            case 2:
            case 3:
              return token.slice(ATTRIBUTES_AND_CHILDREN).filter(function(item){
                return typeMap[item[tmpl.TOKEN_TYPE]] == ATTRIBUTE;
              });
      ...
      getChildren: function(token){
        if (token[tmpl.TOKEN_TYPE] == tmpl.TYPE_ELEMENT)
          ...
          {
            case 1:
              return token[tmpl.ELEMENT_CHILDS];
        ...
        for (var i = stIdx || 0, token; token = ast[i]; i++)
        {
          ...
          if (tokenType == ELEMENT)
          {
            switch (this.version)
            {
              ...
              case 2:
              case 3:
                this.walk(token, handlers, token, ATTRIBUTES_AND_CHILDREN);
                break;
