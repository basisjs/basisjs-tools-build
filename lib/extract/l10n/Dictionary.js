...
var dictionaryInitPriorBasis17 = require('./basisjs-prior-1.7.js');
...
var Dictionary = function(file, flow, dict) {
    ...
    if (typeof dict.getDescriptor != 'function') {
        dictionaryInitPriorBasis17.call(this, file, flow);
        ...
        for (var key in dict.cultureValues[culture]) {
            ...
            if (!descriptor) {
                flow.warn({
                    ...
                    theme: culture,
                    message: 'Descriptor not found: ' + key + ' at ' + culture
                });
                continue;
            }

            if (key in this.tokens == false) {
                var token = this.tokens[key] = new Token(this, key);

                token.culture = culture;
                token.branch = descriptor._sourceBranch;
                token.key = descriptor._sourceKey;
            }

            if (descriptor.types[descriptor.name] != 'markup') {
                continue;
            }
            ...
            this.markupTokens.push({
                name: key,
                branch: descriptor._sourceBranch,
                key: descriptor._sourceKey,
            ...
            // registrate token/resource for every theme
            for (var theme in flow.tmpl.implicitDefine) {
                flow.tmpl.implicitDefine[theme][templateKey] = tokenFile;
            }
            for (theme in flow.tmpl.themeResources) {
                flow.tmpl.themeResources[theme][templateKey] = {
