...
(module.exports = function(flow) {
    ...
    function addToScope(namespaces, className) {
        ...
        for (var i = 0; i < namespaces.length; i++) {
            var scopeId = namespaces[i];

            if (className.substr(0, scopeId.length) == scopeId) {
                if (!usage.scopes[scopeId]) {
                    usage.scopes[scopeId] = {};
                }

                usage.scopes[scopeId][className] = true;
                break;
            }
        }
    }
    ...
    function processTemplateAst(file, ast, idMap, classMap) {
        ...
        if (file.decl.isolate) {
            namespaces.push(file.decl.isolate);
        }

        if (file.styleNSPrefixes) {
            for (var namespace in file.styleNSPrefixes) {
                namespaces.push(file.styleNSPrefixes[namespace].prefix);
            }
        }
                    ...
                    case 'class':
                        ...
                        if (value) {
                            ...
                            value.split(/\s+/).forEach(function(className) {
                                var loc = token.valueLocMap ? token.valueLocMap[className] : token.loc;

                                add(classMap, this.file, className, token, 'tmpl-class', loc);
                                addToScope(namespaces, className);
                            }, this);
                        }
                            ...
                            token[1].forEach(function(binding) {
                                ...
                                classes.forEach(function(className) {
                                    ...
                                    if (anim) {
                                        ...
                                        addToScope(namespaces, className + '-anim');
                                    }
                                }, this);
                            }, this);
                        }
    ...
    function addWarn(map, entry, theme, value) {
        // just filter theme warnings for now
        // TODO: remove when files will be filtered by theme
        if (flow.options.theme && flow.options.theme.indexOf(theme) == -1) {
            return;
        }

        // Temporary solution to avoid warnings in some edge cases
        if (entry.file.ignoreWarnings) {
            return;
        }
    ...
    for (theme in classMap) {
        for (name in classMap[theme]) {
            var removalsList = classMapRemovals[theme] && classMapRemovals[theme][name];

            list = classMap[theme][name];
            inHtml = list.sources.html || list.sources.tmpl;
            inCss = list.sources.style;
            ...
            if (!inHtml && inCss) {
                ...
                usage.classes[name] = true;
            }
        }
    }

    for (theme in idMap) {
        for (name in idMap[theme]) {
            list = idMap[theme][name];
            inHtml = list.sources.html || list.sources.tmpl;
            inCss = list.sources.style;

            // don't warn in this case since id could be used for references only
            // if (inHtml && !inCss)
            //   list.forEach(function(entry){
            //     addWarn(warnNoStyle, entry, theme, '#' + name);
            //   });
        ...
        for (var fn in warns) {
            ...
            var map = fileWarns.reduce(function(res, warn) {
                ...
                if (!hasOwnProperty.call(warnMap, warn.theme)) {
                    ...
                    arrayAddUnique(warnMap[warn.theme], warn.loc);
                }
    ...
    usage.classes = Object.keys(usage.classes);
    usage.scopes = Object.keys(usage.scopes).map(function(key) {
        return Object.keys(usage.scopes[key]);
    });
