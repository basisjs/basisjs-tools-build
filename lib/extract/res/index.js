  ...
  for (var i = 0, file; file = queue[i]; i++)
  {
    switch (file.type)
    {
      ...
      case 'style-block':
        ...
        atCss.walk(file.ast, {
          Url: function(node){
            fconsole.log('Found ' + atCss.translate(node));

            var uri = resolveResourceUri(atCss.unpackUri(node));

            if (uri)
            {
              node.loc = file.location(node.info);
              var imageFile = resolveImage(flow, uri.filename, node, null, {
                file: file,
                loc: node.loc,
                token: atCss.translate(node)
              });

              if (imageFile)
              {
                imageFile.cssResource = true;
                //atCss.Url(imageFile.fileRef + uri.hash, node);
                flow.resLinks.push({
                  type: 'css-url',
                  sourceFile: file,
                  file: imageFile,
                ...
                case 'style':
                  if (!/^b:/.test(this.tokenName(parentToken)) && attributeValue)
                  {
                    ...
                    file.link(flow.files.add({
                      ...
                      ast: atCss.parse(this.tokenValue(token), null, true)
                    }), token);
                  }
                  break;
              }
...
function resolveImage(flow, url, node, baseFile, initiator){
