...
function createScope(file, flow, module) {
    ...
    var basisResource = flow.js.basisScope.resolve(at.parse('basis.resource', true));
    var basisRequire = flow.js.basisScope.resolve(at.parse('basis.require', true));
...
function asset(flow, file, baseURI, scope, args, token, reason) {
    ...
    if (typeof newFilename == 'string') {
        token.loc = file.location(token.loc.start);
...
function astExtend(scope, dest, source) {
    if (dest && source && source.type == 'ObjectExpression') {
        ...
        for (var i = 0, props = source.properties, prop; prop = props[i]; i++) {
...
function processFile(file, flow) {
    ...
    if (file.basisScript) {
        ...
        var createNS = function(path) {
            ...
            token.obj = {
                extend: at.createRunner(function QQQ(token_, this_, args, scope) {
        ...
        function getResourceRunner(token, this_, args, scope) {
            ...
            if (newFilename) {
                token.loc = this.file.location(token.loc.start);
                    ...
                    if (root == namespace) {
                        if (!rootFile) {
                            rootFile = flow.files.add({
                                ...
                                filename: flow.js.rootFilename[root] ||
