      ...
      extend: function(token, this_, args){
        ...
        if (this.file.jsScope == basisScope)
        {
          ...
          if (arg0[0] == 'name' && arg0[1] == 'Object')
          {
            flow.warn({
              fatal: true,
              message: 'Too old basis.js (prior 1.0) detected! Current tools doesn\'t support it. Use basisjs-tools 1.3 or lower.'
            });
            exit(2);
          }
        }
  ...
  else
  {
    ...
    if (file.namespace == 'basis.l10n')
    {
      if (!file.jsScope.resolve(at.parse('basis.l10n.dictionary', 1)))
      {
        flow.warn({
          fatal: true,
          message: 'basis.l10n prior to basis.js 1.0 is not supported'
        });
        exit(2);
      }
