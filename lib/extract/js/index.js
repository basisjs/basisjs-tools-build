...
module.exports = function(flow){
  ...
  globalScope.put('importScripts', 'env', at.createRunner(function(token_, this_, args){
    args.forEach(function(arg, idx){
      var filename = resolveString(this.scope, arg);
  ...
  if (file.type == 'script' && !file.jsScope)
  {
    ...
    var names = {
      ...
      resource: at.createRunner(function(token, this_, args){
        ...
        var filename = resolveString(this.scope, args[0]);
...
function processFile(file, flow){
  ...
  if (file.basisScript)
  {
    ...
    function getResourceRunner(token, this_, args){
      var newFilename = resolveString(this.scope, args[0]);
    ...
    var handlers = {
      ...
      requireNamespace: function(token, this_, args){
        var arg0 = resolveString(this.scope, args[0]);
    ...
    if (file.autoload)
    {
      var processAutoload = function(ns){
        ...
        handlers.requireNamespace.call({ file: file, scope: file.jsScope }, autoRequire, null, [['string', ns]]);
        return autoRequire;
      };
