...
module.exports = function processBasisFile(flow, file, attrValue){
  ...
  if (basis && typeof basis.createSandbox == 'function')
    createBasisInstance = function(file, config){
      return basis.createSandbox(config);
    };
  ...
  // patch basis.template.theme to prevent creating source and find problems in templates
  // TODO: remove when 1.5 will support implicit template source creating
  if (typeof basis.patch == 'function')
  {
    basis.allowImplicitTemplateSourceCreate = true;
    basis.patch('basis.template.theme', function(exports){
      var getSourceByPath = exports.get;
      exports.get = function(path){
        if (basis.allowCreateNewTemplateSources || exports.getPathList().indexOf(path) != -1)
