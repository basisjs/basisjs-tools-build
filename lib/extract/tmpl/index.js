...
(module.exports = function(flow){
  ...
  // switch off source auto-creating for old basis (prior 1.5) on makeDeclaration to find problems
  // since basis.js 1.5 creation of sources could be prevented through options
  flow.js.basis.allowImplicitTemplateSourceCreate = false;
  ...
  themeOrdered.forEach(function(themeName){
    ...
    for (var defineIdx = 0, key; key = defineKeys[defineIdx]; defineIdx++)
    {
      ...
      var declCacheKey = source.get() + path.dirname(source.url) + '/';
      var decl = declCacheKey in declCache
      ...
      // add declaration to cache that has no named parts
      if (decl.hasNamedInclude === false)
