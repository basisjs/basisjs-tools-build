...
(module.exports = function(flow) {
    ...
    flow.js.resources.forEach(function(token) {
        ...
        if (file.type == 'template') {
            if (!token.themeDefined) {
                var templateGet = jsAt.parse('basis.template.get', 1);
                var id;
                ...
                var beforeChanges = jsAt.translate(token);
                ...
                fconsole.log(beforeChanges, '->', jsAt.translate(token));
            } else {
                fconsole.log(jsAt.translate(token), 'already in theme define');
            }
        }
    });
    fconsole.endl();
    ...
    for (themeName in flow.tmpl.themes) {
        themeResources = flow.tmpl.themeResources[themeName];

        for (key in themeResources) {
