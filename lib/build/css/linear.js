...
function buildFile(file, flow, theme, context) {
    var result = {
        type: 'StyleSheet',
        loc: null,
        children: file.ast.children.copy()
    };
    ...
    if (file.used && file.used[theme]) {
        msg = '[DUP] ' + (file.filename ? file.relpath : '[inline style]') + ' ignored as already used';
    ...
    for (var i = file.imports.length - 1, importToken; importToken = file.imports[i]; i--) {
        ...
        result.children
            .replace(importToken.item, injection.children.copy());
    }
