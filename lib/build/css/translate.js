...
module.exports = function(flow) {
        ...
        if (isEmpty && !file.themeUrlsMap) {
            ...
            if (!flow.options.singleFile) {
                ...
                if (flow.js.rootNSFile.basis) {
                    ...
                    ast.splice(ast.length - 1, 0, jsAt.parse(
                        file.theme
                            ? 'basis.resource(resolveNSFilename("basis.template")).ready(function(exports){\n' +
                            '  exports.onThemeChange(function(theme){\n' +
                            '    theme === "' + file.theme.substr(6) + '" ? this.startUse() : this.stopUse();\n' +
                            '  }, basis.require("./' + file.jsRef + '"), true);\n' +
                            '})'
                            : 'basis.require("./' + file.jsRef + '").startUse();', true
                    ));
                } else {
    ...
    if (themeUrlsMap) {
        ...
        htmlAt.injectToHead(file.ast, scriptNode);
    ...
    for (var i = 0; file = flow.files.queue[i]; i++) {
        if (file.type == 'style-block') {
            ...
            if (!outputContent.length && file.htmlFile) {
                ...
                if (file.htmlFile) {
                    delete htmlAt.getAttrs(file.htmlNode).style;
                }
            } else {
                ...
                if (file.htmlFile) {
                    ...
                    htmlAt.getAttrs(file.htmlNode).style = outputContent;
                    continue;
                }
