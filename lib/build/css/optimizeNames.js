...
(module.exports = function(flow) {
            ...
            for (var i = 0, item, token, tokenSaving; item = list[i]; i++) {
                ...
                switch (item.type) {
                    ...
                    case 'tmpl-class-binding':
                        ...
                        break;
                    case 'tmpl-class-anim':
                        // nothing todo
                        break;
                    default:
                        flow.warn({
                            fatal: true,
                            message: 'Unknown token type - ' + item.type
                        });
                }
    ...
    // update usage
    if (flow.css.usage) {
        flow.css.usage.classes = flow.css.usage.classes.map(getClassNameReplace);
        flow.css.usage.scopes.forEach(function(list, idx, scopes) {
            scopes[idx] = scopes[idx].map(getClassNameReplace);
        });
    }
