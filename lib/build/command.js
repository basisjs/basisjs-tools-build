...
function normOptions(options) {
    common.normalize(options, 'build');
    ...
    if (options.jsBundle) {
        if (options.jsBundle === true) {
            options.jsBundle = 'js';
        }

        if (jsBundleFormats.indexOf(options.jsBundle) === -1) {
            console.warn('Wrong value for `jsBundle` ignored, used `js`');
            options.jsBundle = false;
        } else {
            options.jsBundleName = options.jsBundleName || path.basename(options.file, path.extname(options.file));
            options.singleFile = true;
        }
    }

    if (options.singleFile) {
        options.cssInlineImage = 1e10;
    } // make sure all images are inline
...
module.exports = clap.create('build', '[fileOrPreset]')
    ...
    .option('--single-file', 'Produce a single js file that includes all resources if possible (build fails otherwise)')
    ...
    .option('--js-bundle [format]', 'Make a JavaScript bundle. Format (optional): js (by default), json', function(format) {
        if (format !== undefined && jsBundleFormats.indexOf(format) === -1) {
            throw new clap.Error('Wrong value for --js-bundle option: ' + format);
        }
    ...
    .option('--css-usage', 'Use usage data for better CSS compression (have effect only when --css-pack is used)')
