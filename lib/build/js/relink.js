          ...
          CallExpression: function(token){
            switch (token.replaceFor_)
            {
              case 'basis.resource':
                ...
                if (file && file.jsRef)
                {
                  token.callee = utils.memberExpression('basis', 'resource');
              ...
              case 'basis.require':
                ...
                token.callee = utils.memberExpression('basis', 'require');
                token.arguments = [utils.createLiteral('./' + token.resourceRef.jsRef)];

                break;

              case 'basis.template.get':
                token.callee = utils.memberExpression('basis', 'template', 'get');
