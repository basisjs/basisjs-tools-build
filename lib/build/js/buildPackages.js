...
module.exports = function(flow){
  ...
  if (basisFile)
  {
    ...
    Array.prototype.splice.apply(configToken,
      [0, 10].concat(
        basisFile.jsScope.has('__config')
          ? at.parse('__config || ' + JSON.stringify(basisFile.config))[1][0][1]
          : at.parse('0,' + JSON.stringify(basisFile.config))[1][0][1][2]
      )
    );
  ...
  var scriptNamePrefix = flow.options.sameFilenames ? path.basename(flow.options.file, path.extname(flow.options.file)) : 'script';
    ...
    switch (package.layout){
      case 'basis':
        ...
        var packageFile = flow.files.add({
          ...
          outputContent:
            (isCoreFile
              ? '// resources (' + resourceToc.length + '):\n//   ' +
                resourceToc.join('\n//   ') +
                '\n//\n'
              : '') +
            (flow.options.jsOptimizeThrows && throwCodes.length
              ? '// throw codes:\n//  ' +
                throwCodes.concat(resourceThrows).map(function(item){
