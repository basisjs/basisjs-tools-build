...
(module.exports = function(flow){
  ...
  //
  fconsole.start('Relink basis.l10n.patch()');
  for (var dictPath in flow.l10n.patches)
  {
    var patches = flow.l10n.patches[dictPath];

    for (var patchPath in patches)
      for (var i = 0, ref; ref = patches[patchPath].ref[i]; i++)
        if (ref.refToken)
        {
          var old = at.translate(ref.refToken);
          ref.refToken[2][0] = ['string', './' + patches[patchPath].dictionary.file.jsRef];
          ref.refToken[2][1] = ['string', './' + patches[patchPath].file.jsRef];
          fconsole.log(old + ' -> ' + at.translate(ref.refToken));
        }
  }
  fconsole.endl();
